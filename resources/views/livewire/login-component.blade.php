<div class="p-6 space-y-5">
    ...
    @if (session('success'))
        <div class="pt-5">
            <div
                class="flex items-center gap-3 bg-primary-fixed/10 border border-primary-fixed/30 text-primary-fixed rounded-xl px-4 py-3 text-sm">
            ...
            <div class="space-y-4">
                <div>
                    <label for="login-email"
                        class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">Email or
                        Username</label>
                    <div class="relative">
                        <span
                            class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[18px]">person</span>
                        <input id="login-email" type="text"
                            ...
                            wire:model.blur="email" placeholder="Enter email or username" autocomplete="username"
                            autofocus>
                    </div>
                    @error('email')
                ...
                <div>
                    <label for="login-password"
                        class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">Password</label>
                    <div class="relative" x-data="{ show: false }">
                        <span
                            class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[18px]">lock</span>
                        <input id="login-password" type="password" :type="show ? 'text' : 'password'"
                            ...
                            wire:model="password" placeholder="Enter password" autocomplete="current-password">
                ...
                <div class="flex items-center gap-2">
                    <input id="login-remember" type="checkbox"
                        class="w-4 h-4 rounded border-outline-variant bg-surface-container accent-primary-fixed"
                        wire:model="remember">
                    <label for="login-remember" class="text-sm text-on-surface-variant select-none cursor-pointer">Keep
                        me signed in</label>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-1">
                <button type="button"
                    ...
                    <span wire:loading.remove wire:target="login" class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[18px]"
                            style="font-variation-settings:'FILL' 1;">login</span>Sign In
                    </span>
        ...
        <div class="space-y-5">
            ...
            <div>
                <label for="magic-email"
                    class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">Email
                    Address</label>
                <div class="relative">
                    <span
                        class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-on-surface-variant text-[18px]">mail</span>
                    <input id="magic-email" type="email"
                        class="w-full h-12 bg-surface-container pl-10 pr-4 rounded-xl border border-outline-variant text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary-fixed transition-colors @error('email') border-error @enderror"
                        wire:model.blur="email" placeholder="you@example.com" autocomplete="email" autofocus>
                </div>
                @error('email')
                    <p class="text-xs text-error mt-1.5 flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px]">error</span>{{ $message }}
            ...
            <div class="space-y-3 pt-1">
                <button type="button"
                    class="w-full h-12 bg-primary-fixed text-on-primary-fixed rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50"
                    wire:click="link" wire:loading.attr="disabled">
                    <span wire:loading.remove wire:target="link" class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-[18px]"
                            style="font-variation-settings:'FILL' 1;">send</span>Send Magic Link
                    </span>
